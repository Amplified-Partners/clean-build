...
- **Where encoded**: `02_build/enforcer/` (full tree merged from source repo); `00_authority/MANIFEST.md` v52 (Candidate authority entry + changelog); this entry. Multiple Devin-Review re-pass fixes also landed in this PR: corrected `curl` healthcheck to `python/httpx` (matches Alpine image, in both `docker-compose-entry.yml` and `ENFORCER-SPEC.md`); corrected `enforcer_checks_total` Prometheus type from `counter` to `gauge`; replaced unreachable `isinstance(result, CheckResult)` with `hasattr(result, 'duration_ms')`; fixed Traefik unreachable from `warn` to `fail/critical` per spec; aligned README's "Monitored Services" with the actual `EXPECTED_CONTAINERS` list (replacing the inaccurate "38/40 containers" claim and the non-monitored Temporal/MinIO entries); added per-file signatures to all six Python source files plus `.gitignore`, `.dockerignore`, `requirements.txt`, and `checks/__init__.py`; created the previously-referenced-but-missing `.dockerignore`; added the missing `/opt/amplified/vault:ro` volume to `docker-compose-entry.yml` and the missing `user: "1000:988"` directive (without it the container can't read the Docker socket); changed `docker-compose.yml` vault mount from `:rw` to `:ro` to match `docker-compose-entry.yml`. Two pre-existing source-repo behaviour defects flagged in `02_build/enforcer/README.md` "Known Issue" — async/sync I/O mismatch and ignored `fail2ban_enabled` flag — and a third one is now also flagged: `enforcer.py` imports `yaml` but never loads `config.yaml`; `EnforcerConfig` reads exclusively from environment variables, so editing `config.yaml` has no runtime effect.
- **Status**: candidate (pending Ewan review of the PR; merge unblocks archival of `Amplified-Partners/enforcer`).
