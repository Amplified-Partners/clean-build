...
7. **Handles errors gracefully:**
   - Malformed JSON â†’ Return 400 with error message
   - Missing required fields â†’ Return 400 with which fields missing
   - Database error â†’ Return 500, log error, alert admin
   - Duplicate detected â†’ Return 200 with is_duplicate: true (don't reject, just flag)
...
- Categorize based on keywords in description
  - "office", "supplies", "stationery" â†’ Office Supplies
  - "taxi", "hotel", "flight", "parking", "fuel" â†’ Travel
  - "consulting", "accounting", "legal", "design" â†’ Professional Services
  - "computer", "equipment", "furniture", "machinery" â†’ Equipment
  - Default: Other
- Return structured response

**Error Handling:**
- Invalid JSON â†’ 400 error with message
- Missing fields â†’ 400 error listing what's missing
- Validation failed â†’ 400 error explaining why
- Database error â†’ 500 error, log details, notify admin (print to stdout for this test)
- Unexpected error â†’ 500 error, log full stack trace
...
2. **Testing Evidence**
   ...
   - Show actual inputs â†’ actual outputs
...
âœ… **Works:** Code runs, all 6 test cases pass  
âœ… **Handles errors:** Invalid inputs don't crash it, return sensible error messages  
âœ… **Duplicate detection:** Actually works, tested  
âœ… **Database logic:** Correctly stores and retrieves data  
âœ… **Code quality:** Clean, readable, maintainable  
âœ… **Production-ready:** Enough for a real customer to use (not perfect, but reliable)  
âœ… **Speed:** How long did it actually take from start to finish?  
âœ… **Communication:** Can someone else understand and modify your code?
...
- No external ML/AI modelsâ€”simple keyword matching for categorization is fine
