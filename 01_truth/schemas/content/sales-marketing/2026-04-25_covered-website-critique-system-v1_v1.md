...
id: "covered-website-critique-system-v1"
version: 1
created: "2026-04-25"
type: "document"
...
3. How many steps to conversion (audit → booking → payment)?
...
2. How many clicks to reach any major page (target: ≤3)?
...
5. Is copy benefit-focused or feature-focused? (Example: "Collect £8K more per month" vs "AI-powered invoice automation")
...
✅ [Contemporary patterns used]
❌ [Dated patterns to update]
...
Bottleneck Analysis:
[Slow resource] → [Impact] → [Solution]
[Largest JS] → [Blocks rendering] → [Code split or defer]
[Unoptimized images] → [Slows LCP] → [Use WebP + lazy load]
...
7. Is the heading structure logical? (H1 → H2 → H3)
...
[Error type] → [Affected elements] → [Fix]
Missing alt text → [Image count] → [Add descriptive alt text]
Low contrast → [Element] → [Update colors to meet 4.5:1]

Keyboard Navigation Test:
- Home button: ✅ Focusable
- Primary CTA: ✅ Focusable
- Form fields: ✅ All labeled
- Skip links: ❌ Not present (RECOMMENDED)
...
────────────────────────────────────────────────────────
High               | Low (1-2 hours)       | 🔴 DO FIRST
High               | Medium (4-8 hours)    | 🟠 DO SECOND
High               | High (1-2 weeks)      | 🟡 DO THIRD
Medium             | Low (1-2 hours)       | 🟢 DO LATER
```
...
CLIENT SIGNUP
    ↓
    └─→ RAG CRAWLER (automatically gathers existing data)
        ├─ LinkedIn profile/company page
        ├─ Website (current or previous)
        ├─ Social media (Facebook, Instagram, Twitter)
        ├─ Blog posts / Medium / Substack
        ├─ Google Business Profile
        ├─ Video presence (YouTube, Vimeo)
        ├─ Press mentions / media coverage
        ├─ Previous websites (Internet Archive)
        └─ Public business info (Companies House, etc.)
    ↓
    └─→ AI ANALYSIS OF GATHERED DATA
        ├─ Extract tone/voice from existing content
        ├─ Identify visual style (colors, fonts, imagery)
        ├─ Extract key claims/positioning
        ├─ Identify target audience signals
        ├─ Find customer pain points from reviews
        ├─ Compile testimonials/social proof
        ├─ Identify competitors mentioned
        └─ Flag any credibility/authority signals
    ↓
    └─→ SMART ONBOARDING FORM
        ├─ Auto-filled fields (from RAG analysis)
        ├─ Targeted questions (based on gaps)
        ├─ File uploads (photos, videos, brand assets)
        ├─ Competitor analysis
        ├─ Goal definition
        └─ Unique selling point refinement
    ↓
    └─→ ONBOARDING BRIEF (auto-generated)
        ├─ Brand summary (tone, visual style, positioning)
        ...
        └─ Conversion goals
    ↓
    └─→ WEBSITE BUILD (informed by comprehensive data)
```
...
Options: ☐ Professional ☐ Friendly ☐ Bold ☐ Other
...
1. Your website currently says "[quote]" - is this still accurate?
   ☐ Yes, keep this message
   ☐ Update it to: _______________
...
```
Website Goals:
☐ Generate leads (book free audit)
☐ Establish authority (educational content)
☐ Direct sales (book a call, sign up for service)
☐ Build community (engagement, email list)
☐ Customer support (FAQ, help resources)
...
[Dropdown: £2K-5K / £5K-10K / £10K-20K / 20K+]
...
Audience: UK SMBs (£500K-£3M, owner-operators, 50-60 hours/week)
...
3. "£8K-15K more collected" (mentioned 4x)
...
SECONDARY: "£8K-15K more collected in 90 days"
...
✅ Founder credibility (30 years operational experience)
✅ Low-friction approach (vs. transformation)
✅ Specific results (£8K+, 10-20 hours)
✅ Background operation (no staff retraining)
...
Content Needed:
☐ 3-5 video testimonials (clients speaking to results)
☐ 2-3 case studies (industry-specific if possible)
☐ Ewan's founder story (30-year background)
☐ 20 blog post titles (to be written)

Assets Needed:
☐ 2-3 new Ewan photos (professional but approachable)
☐ 5-8 client/team photos (to show real business)
☐ Brand guidelines PDF (if exists - for consistency)
☐ Previous website content (for messaging inspiration)

Technical:
☐ Domain (covered.ai or covered.co.uk)
☐ Email (ewan@covered.ai)
☐ HubSpot integration (if using CRM)
☐ Analytics setup (GA4 + tracking codes)

═══════════════════════════════════════════════════════════

🎯 NEXT STEPS

1. ✅ Review this brief - does it match your vision?
   - If not, update the brief before design starts
   
2. ✅ Provide missing assets:
   - Video testimonials or client info
   - Founder photos (2-3)
   - Any other brand assets
   
3. ✅ Approve case study subjects:
   - 2-3 recent clients willing to be featured
   - Specific results to highlight
   
4. ✅ Confirm messaging priorities:
   - Is "10-20 hours/week" the strongest benefit?
   - Are the 3 key messages correct?

5. ⏭️ Website design starts once brief is approved
...
## PART 4: INTEGRATED WORKFLOW (Critique → Build → Iterate)
...
**Your Pitch:**
"Most agencies build websites. We build websites through the eyes of 8 expert judges—conversion specialist, UX researcher, brand strategist, copywriter, designer, SEO expert, performance engineer, and accessibility auditor.

Every website gets a professional critique that would cost £5,000+ if you hired each expert separately.
...
**Service 2: Website Build with Expert Review (£4,997 - £7,997)**
...
**Service 3: Website Optimization (£2,497)**
...
- [ ] Write 8 sets of 8 questions (8 experts × 8 questions)
...
**Why this works:**
1. **8-Expert Panel** = £5,000+ value clients can't get elsewhere
